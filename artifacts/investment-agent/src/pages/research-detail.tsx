...
function MetricCell({ label, value }: { label: string; value?: string }) {
  ...
  return (
    <div className="flex flex-col gap-0.5 p-3 rounded-lg bg-secondary/30 border border-border/30">
      ...
      <span className={`text-sm font-bold font-mono ${isNA ? "text-muted-foreground" : "text-black"}`}>
        {isNA ? "—" : value}
      </span>
    </div>
  );
    ...
    <div className="min-h-screen pb-16">
      ...
      <div className="border-b border-border/40 bg-background/90 backdrop-blur sticky top-0 z-10">
        <div className="container max-w-5xl px-4 md:px-8 h-12 flex items-center">
          <Link href="/" className="inline-flex items-center text-xs font-mono text-muted-foreground hover:text-black transition-colors gap-1.5">
          ...
          <div className={`rounded-2xl border-2 p-6 md:p-8 ${isInvest ? "border-invest/50 bg-invest/5 shadow-invest/10" : "border-pass/50 bg-pass/5 shadow-pass/10"} shadow-2xl`}>
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
              <div className="space-y-3 flex-1 min-w-0">
                ...
                <h1 className="text-4xl md:text-6xl font-black tracking-tight text-black leading-none">{research.company}</h1>
                {companyInfo && (
                  <div className="flex flex-wrap gap-2 pt-1">
                    {companyInfo.ticker && companyInfo.ticker !== "N/A" && (
                      <span className="px-2 py-0.5 rounded bg-white/10 text-xs font-mono font-bold text-black">{companyInfo.ticker}</span>
                    )}
        ...
        {!isCompleted && !isFailed && (
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-4xl font-black tracking-tight text-black">{research.company}</h1>
            ...
            <Card className="bg-card/40 border-border/50">
              ...
              <CardContent className="pt-4 space-y-3">
                {PIPELINE_STEPS.map((label, idx) => {
                  ...
                  return (
                    <div key={label} className={`flex items-center gap-3 text-sm font-mono transition-colors ${done ? "text-invest" : active ? "text-black animate-pulse" : "text-muted-foreground"}`}>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                  ...
                  {stockData.price && stockData.price !== "N/A" && (
                    <div className="flex flex-wrap items-end gap-4 pb-4 border-b border-border/40">
                      <div>
                        ...
                        <p className="text-4xl font-black font-mono text-black">{stockData.price}</p>
                      </div>
            ...
            <Card className="bg-card/60 border-border/60">
              ...
              <CardContent className="pt-5">
                <p className="text-base md:text-lg leading-relaxed text-slate-800 font-light">{research.summary}</p>
              </CardContent>
            </Card>
                  ...
                  {news.map((item, idx) => (
                    <div key={idx} className={`flex items-start gap-3 p-3 rounded-lg border ${
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-slate-700 leading-snug">{item.headline}</p>
                        <p className="text-[10px] font-mono text-muted-foreground mt-1 uppercase">{item.sentiment}</p>
                      </div>
                      {item.url && item.url !== "" && (
                        <a href={item.url} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-black transition-colors shrink-0 mt-0.5">
                      ...
                      return (
                        <a key={idx} href={source} target="_blank" rel="noreferrer"
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className="text-sm font-mono text-muted-foreground group-hover:text-black truncate transition-colors">{domain}</span>
                          </div>
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs font-mono text-muted-foreground">
                  {["Your Query", "LangGraph Agent", "Tavily Web Search", "GPT-4o Reasoning", "Score Weighting", "Investment Verdict"].map((step, idx, arr) => (
                    <span key={step} className="flex items-center gap-2">
                      <span className="px-2 py-1 rounded bg-secondary/60 border border-border/40 text-slate-700">{step}</span>
